...
The checked-in Compose files cap the Polarion container at `4g` RAM and persist the Polarion workspace to avoid unnecessary full reindexing after container recreation. `docker-compose.yml` defaults the JVM to `-Xmx3g -Xms3g`, while the dev file `docker-compose.dev.yml` uses a lighter `-Xmx1536m -Xms1536m`. `polarionctl start` applies runtime-aware memory defaults which may set the heap automatically (`-Xmx3g` on standard Docker, `-Xmx2560m` on Apple container).
...
| :-------------- | :------------------------------------------- | :---------------------------- |
| `JAVA_OPTS`     | Java memory and VM arguments                 | `-Xmx3g -Xms3g` (`-Xmx1536m` in dev Compose); runtime-aware via `polarionctl` |
...
- **Memory:** Docker, Compose, and Apple `container` all cap the container at `4g` RAM. `docker-compose.yml` defaults the JVM to `-Xmx3g -Xms3g` (the dev file uses `-Xmx1536m -Xms1536m`); `polarionctl start` applies runtime-aware heap defaults (`-Xmx3g` on Docker, `-Xmx2560m` on Apple container). Override with `JAVA_OPTS` as needed.
